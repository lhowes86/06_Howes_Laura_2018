...
#####My assumptions of part c are: the sample is random, and that the measurements have a bivariate (3D) normal distribution. This means the relationship between greymatter and proficiency is linear, the cloud of points in a scatter plot has a circular or elliptical shape, and the frequency distributions of greymatter and proficiency separately are normal.
...
```
#####So final Correlation Coefficient is -0.8563765
...
#####I created a CSV file, which is in my Github repository 
...
```
#####The output of cor() is the correlation coefficiant. The Correlation coefficient is 0.6758738. The strongest correlation coefficients are r = 1.0 or -1.0, so this value shows a somewhat strong positive correlation.
...
#####Then I need to divide 0.4176406 by 1.96 (since 1.96*2SE = CI). 0.4176406/1.96

#####SE of the correlation = 0.2130819


####2c.Now, what is the SE via simulation? To do this, you'll need to use cor() and get the relevant parameter from the output (remember - you get a matrix back, so, what's the right index!), replicate(), and sample() or dplyr::sample_n() with replace=TRUE to get, let's say, 1000 correlations. How does this ## [1] 0.1524226 compare to your value above?
...
nutrients_plot
```
#####The explanatory variable (x) should be nutrients and the response variable (y) should be species, because the amount of species present is dependent on how many nutrients there are.
...
nutrients_plot_lsqs
```
#####R-squared is a statistical measure of how close the data are to the fitted regression line. R^2 from summary(nutrients_mod) = 0.536, so 53.6% is explained. 
...
portions_plot_lsqs
```
#####The most plausible range of values for the slope of this relationship are within the grey area, which are the 95% Confidence Interval Bands.

#####To calculate the actual plausible range of slope (95% Confidence Interval Bands):
...
#####My regression line from 31a: y = 0.003329x - 1.168842 To test for my regression line, I will plug in an x value form the portions table and see if it equals the y from the table:
...
```
#####The residual plot shows the data doesn't nicely fit around the horizontal line (it fits better with the red line). To address the problem we could do a log transformation of the data.
...
```{r eval=FALSE}
deet <- read_csv("./17q24DEETMosquiteBites.csv") #I added deet data from previous lab into my github repository
...
####4a. Fit simulations. Using geom_abline() make a plot that has the following layers and shows that these simulated lines match up well with the fit CI: 1) the data, 2) the lm fit with a CI, and 3) simulated lines. You might have to much around to make it look as good as possible.

```{r eval=FALSE}
#simulation data
sim_lines <- data.frame(rmnorm(100, mean = coef(deet_mod), varcov = vcov(deet_mod)))
...
deet_plot_geom_abline <- ggplot(data = deet, mapping = aes(x = dose, y = bites)) +
  geom_point() +
  stat_smooth(method= "lm") + 
  geom_abline(slope = sim_lines$dose, intercept = sim_lines$X.Intercept., color = "red")
...
####4b. Prediction simulations. That's all well and good, but what about the prediction intervals? To each line, we can add some error drawn from the residual standard deviation. That residual can either be extracted from summary() or you can get the sd of residuals. Now, visualize the simulated prediction interval around the fit versus the calculated prediction interval around the fit via predict. +1 extra credit for a clever visualization of all elements on one figure - however you would like

```{r eval=FALSE}

sim_lines_resid <- sim_lines %>%
  mutate(error = rnorm(n(), 0, sd(deet_mod$residuals)))

head(sim_lines_resid)

deet_plot_geom_abline_with_sim_lines_resid <- ggplot(data = deet, mapping = aes(x = dose, y = bites)) +
  geom_point() +
  stat_smooth(method= "lm") + 
  geom_abline(slope = sim_lines$dose, intercept = sim_lines$X.Intercept., color = "red") +
  geom_abline(data = sim_lines_resid, mapping = aes(slope = dose, intercept = X.Intercept.,   color="blue"))

deet_plot_geom_abline_with_sim_lines_resid #this in my best attempt!
